...
extern uint8_t small_panda_data[64*64];   // output grayscale downscaled image
...
void rgb565_to_gray64(void)
{
    ...
    const int x_scale = (SRC_W * 1000) / DST_W; // 160000/64 = 2500
    const int y_scale = (SRC_H * 1000) / DST_H; // 120000/64 = 1875
    ...
    for (int dy = 0; dy < DST_H; dy++)
    {
        ...
        for (int dx = 0; dx < DST_W; dx++)
        {
            ...
            small_panda_data[dy * DST_W + dx] = y;
        }
    }
}
...
int main ( void )
{
    ...
    while ( true )
    {
        ...
        if (frame_ready) {
            ...
            DCACHE_CLEAN_BY_ADDR((uint32_t *)small_panda_data, 4096);
